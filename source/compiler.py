...
class WRECK(object):
	...
	#NUMBERING TRIGGERS IN MISSION TEMPLATES
	trigger_ID = 0
...
def process_map_icons(e, index):
	...
	for trigger, code_block in e[8]:
		try: output.append('%f  %s ' % (trigger, parse_module_code(code_block, 'icon.%s(#%d).%s' % (e[0], index, simple_trigger_to_string(trigger)))))
...
def parse_module_code(code_block, script_name, check_can_fail = False):
	...
	for unused_local in locals_def - locals_use:
		...
		if tag == 'mt':
			trigger = script_name.split('.')[2]
			block = script_name.split('.')[3]
			num = len(code_block)
			if not (unused_local, trigger, block, num) in WRECK.local_notice:
				WRECK.notices.append('local l.%s declared but never used in %s' % (unused_local, script_name))
				WRECK.local_notice.append((unused_local, trigger, block, num))
		else:
			WRECK.notices.append('local l.%s declared but never used in %s' % (unused_local, script_name))
