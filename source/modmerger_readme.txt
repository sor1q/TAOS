ModMerger Additions, 15 Dec 2011
by Caba'drin

Added two wrappers: one for GameMenuOptions and the second for Dialogs.
Four additional fucntions were added: one to find a specific dialog similar 
to FindTrigger (though it isn't within a class) and two functions (one each
for FindTrigger and FindDialog) to find the 'i', list position of a trigger/dialog
to allow others to be added above or below that point (in util_wrappers). 
The fourth function finds the first "i" list item whose ID contains a value, 
rather than is equal to a value (in util_common).

util_animations was added as well, which can match animation IDs and replace
animations, or find the first available "empty" human/horse animation and
place the new animations there.
