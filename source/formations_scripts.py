...
# #Formations Scripts	  
  ...
  ("get_default_formation", [
	...
	(else_try),
		...
		(assign, reg0, formation_ranks),
  (else_try),
		(eq, ":ffaction", fac_kingdom_7),	#Sarranid
		(assign, reg0, formation_shield),
	(else_try),
	    (this_or_next|eq, ":ffaction", fac_player_supporters_faction),
...
# #Utilities used by formations
  ...
  ("store_battlegroup_data", [
	...
	(try_for_agents, ":cur_agent"),
        ...
        (assign, ":closest_distance", Far_Away), #TEST
		(agent_set_slot, ":cur_agent", slot_agent_nearest_enemy_agent, -1),
		(assign, ":fp_closest_dist", AI_charge_distance / 100), #TEST - the only place a distance *larger* than AI_charge_distance is used to check is to decide between BP_Setup and BP_Jockey. All other decisions based on agent-distances are on smaller than AI_charge_dist
		(convert_to_fixed_point, ":fp_closest_dist"), #TEST
		(try_for_agents, ":enemy_agent", pos1, ":fp_closest_dist"), #WSE #TEST additions
		...
		(try_begin),
			...
			#(get_distance_between_positions, ":new_distance", pos0, pos1),
			(assign, ":new_distance", ":closest_distance"),
...
#Wuan   Manual modification to avoid duplicate scripts
#	process_script_directives(orig_scripts, scripts_directives)
	...
	try:
        ...
        #Wuan   "if pos != -1: "is needed
        #opblock.SetLineContent(pos, (call_script, "script_get_order_destination_for_map", pos1, ":player_team", grc_infantry))
		if pos != -1: opblock.SetLineContent(pos, (call_script, "script_get_order_destination_for_map", pos1, ":player_team", grc_infantry))
		...
		if pos != -1: opblock.SetLineContent(pos, (call_script, "script_get_order_destination_for_map", pos1, ":player_team", grc_archers))
		...
		if pos != -1: opblock.SetLineContent(pos, (call_script, "script_get_order_destination_for_map", pos1, ":player_team", grc_cavalry))
	except:
		import sys
